...
public class YouTubeSearchExtractor extends YouTubeExtractor {
    ...
    @Override
    public void process(String input, Player player, Message message, Guild guild, Member member) throws Exception {
        JSONArray results = Unirest.get(String.format("https://www.googleapis.com/youtube/v3/search" +
                ...
                URLEncoder.encode(input, "UTF-8"), Dragon.getConfig().getGoogleAPIKey())).asJson().getBody()
        ...
        if (!PermissionsUtil.isTrusted(guild, member)) {
        ...
        super.process(link, player, message, guild, member);
    }
}
